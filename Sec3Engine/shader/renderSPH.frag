...
varying vec3 normal;
void main(void) {
	vec3 u_lPos = vec3( 0.0, 2.0, 0.0);
	vec3 toLight = (u_lPos - worldPosition); 
	float falloff = 20.0 / (dot(toLight, toLight));
	toLight = normalize(toLight);
	float lambertTerm = max(dot(normalize(normal), toLight), 0.0);
	lambertTerm *= falloff;
	lambertTerm = clamp( lambertTerm, 0.06, 1.0);
	...
	gl_FragData[0] = sqrt(vec4( lambertTerm * 0.002 * (length(testColor.rgb)) * normalize(testColor.rgb), 1.0));
